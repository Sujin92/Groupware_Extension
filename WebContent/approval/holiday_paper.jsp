			...
			if (httpRequest.status == 200) {
				...
				for (i in txt.info) {
					if (txt.info[i].dept_num !== dept) {
						...
						if (i != 0)
							x += "</ul>";
						x += "<ul style=\"list-style-type: none\"><b>"
								+ txt.info[i].dept_num + "</b>";

					}
					//                <a href=\"writeModal('" + txt.info[i].emp_num + "')\"</a>
					//                    x += "<li id='" + txt.info[i].emp_num + "'>" + txt.info[i].position_num + " " + txt.info[i].name + "</li>";

					x += "<li onclick='getMember("
							+ JSON.stringify(txt.info[i]) + ")' id='"
							+ txt.info[i].emp_num + "'>"
							+ txt.info[i].position_num + " " + txt.info[i].name
							+ "</li>";
				}
				x += "</ul>";
	...
	function displayModalDept() {
		if (httpRequest.readyState == 4) {
			if (httpRequest.status == 200) {
				...
				var x = "";
				for (i in txt.info) {
					x += "<input type='checkbox' name='dept'>"
							+ txt.info[i].dept_name + "&nbsp;";
				}
...
<script type="text/javascript"
	src="/moa/editor/js/service/HuskyEZCreator.js" charset="EUC-KR"></script>
<div id="page-wrapper">
	<div class="row">
		<div class="col-lg-12">
			...
			<div class="col-lg-9">
				...
				<div align="center" style="font-size: 30px">휴가 신청서</div>
				<form name="paperForm" method="post" action="">
					...
					<table class="table table-bordered">
						<tr>
							<th>문서번호</th>
							<td>123-333<input type="checkbox">문서번호 등록
							</td>
							<th rowspan="2">결재</th>
						...
						<tr>
							<th>기안일</th>
							<td><input type="text" id="year" name="year" size="7">년
								<input type="text" id="month" name="month" size="7">월
								<input type="text" id="day" name="day" size="7">일</td>
							<th>수신부서</th>
							<td colspan="3">
						...
						<tr>
							...
							<select name="holiday_finish1">
								...
								%>
							</select>년 <select name="holiday_finish12">
								<option value="">----</option>
								<%
									for (int i = 1; i < 13; i++) {
										if (i < 10) {
