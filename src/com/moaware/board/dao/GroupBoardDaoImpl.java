...
public class GroupBoardDaoImpl implements GroupBoardDao {
	...
	private static GroupBoardDao groupboardDaoImpl;
	
	static{
		groupboardDaoImpl = new GroupBoardDaoImpl();
	}
	
	private GroupBoardDaoImpl(){}
	
	public static GroupBoardDao getGroupboardDaoImpl() {
		return groupboardDaoImpl;
	}
	...
	public List<BoardDto> listArticle(Map<String, String> map) {
		...
		try {
			...
			sql.append("				                seq, emp_num,dept_num, name , subject, content, hit, \n");
			...
			int idx=0;
			pstmt.setString(++idx, map.get("dept"));
			if(!key.isEmpty() && !word.isEmpty())
	...
	public int writeArticle(BoardDto boardDto) {
		...
		try {
			...
			StringBuffer sql = new StringBuffer();
			sql.append("insert \n");
			sql.append("	into departboard (seq, emp_num, dept_num, name, subject, content, hit, logtime) \n");
			...
			pstmt.setString(++idx, boardDto.getDept_num());
	...
	public int modifyArticle(BoardDto boardDto) {
		...
		try {
			...
			sql.append("update departboard set subject =?, content =? \n");
			sql.append("where seq = ?");
			pstmt = conn.prepareStatement(sql.toString());
	...
	public int deleteArticle(int seq) {
		...
		try {
			...
			board_sql.append("delete departboard where seq = ?");
