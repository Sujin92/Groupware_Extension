...
public class GroupBoardWriteAction implements Action {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		if(memberDto != null) {//로그인 했다면.
			...
			boardDto.setDept_num(memberDto.getDept_num());
			boardDto.setName(memberDto.getName());
			boardDto.setSubject(request.getParameter("subject"));
			boardDto.setContent(request.getParameter("content"));
			boardDto.setBcode(NumberCheck.nullToZero(request.getParameter("bcode")));
			
			int cnt = GroupBoardServiceImpl.getGroupboardService().writeArticle(boardDto);
			if(cnt != 0) {
				request.setAttribute("seq", seq + "");
				path = "/groupboard/writeOk.jsp";
			} else {
				path = "/groupboard/writeFail.jsp";
			}
		}
